...
func main() {
	...
	defer cancel()
	r, err := client.DelayedResponse(ctx, &grpcbin.DelayRequest{DelaySeconds: 3, Message: "Delayed Hello!"})
	if err != nil {
		fmt.Println(errors.Wrap(err, "getting delayed response"))
		os.Exit(1)
	}
	fmt.Printf("DelayedResponse: %s\n", r.Message)
}
