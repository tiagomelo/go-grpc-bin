...
func main() {
	...
	defer cancel()
	_, err = client.SimulateError(ctx, &grpcbin.ErrorRequest{Code: 500, Message: "Simulated internal server error"})
	if err != nil {
		st, ok := status.FromError(err)
		if ok {
			fmt.Printf("SimulateError: %v\n", st.Message())
		} else {
			fmt.Println(errors.Wrap(err, "simulating error"))
			os.Exit(1)
		}
	}
}
